...
#Replace PART with a list of partitions where programs can be executed from the system
...
if grep "^$ModLoad" /etc/rsyslog.conf; then
	sed -i "s/\($ModLoad \).*/\1imtcp/" /etc/rsyslog.conf
else
    echo "$ModLoad imtcp" >> /etc/rsyslog.conf
fi
if grep "^$InputTCPServerRun" /etc/rsyslog.conf; then
	sed -i "s/\($InputTCPServerRun \).*/\1514/" /etc/rsyslog.conf
else
...
service auditd reload
sort /etc/audit/audit.rules | uniq > /etc/audit/temp.rules
sort /etc/audit/temp.rules | uniq > /etc/audit/audit.rules
rm -rf /etc/audit/temp.rules
